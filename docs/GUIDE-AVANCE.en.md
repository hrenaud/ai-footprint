...
or Space. Its collapsed state persists between sessions. A click shows a
confirmation toast, while keyboard toggles remain silent. It shows the
installed version and one line per indicator; an extrapolated-model warning may
be shown. The warning names the recent model and its fallback version for any
provider, for example `gpt-5.6-terra` using `gpt-5.5` parameters.
