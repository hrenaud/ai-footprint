// agent-carbon-crush.js — Plugin Opencode pour agent-carbon.
//
// Ce plugin écoute la fin de session (session.idle) et écrit les exports de
// session dans ~/.agent-carbon/crush-exports/<sessionId>.json pour ingestion
// automatique par agent-carbon.
//
...
async function exportSession(client, sessionId) {
  ...
  try {
    ...
    console.log(
      `[agent-carbon-crush] Exported session ${sessionId} → ${outPath}`
    );
  } catch (err) {
    console.error(
      `[agent-carbon-crush] Failed to export session ${sessionId}: ${err.message}`
    );
  }
}
 ...
 */
module.exports = {
  name: "agent-carbon-crush",
  description: "Exporte les sessions Opencode/Crush pour agent-carbon.",
  version: "1.0.0",
  ...
  register(context) {
    ...
    client.on("session.idle", async (sessionId) => {
      console.log(
        `[agent-carbon-crush] Session idle: ${sessionId} — export en cours...`
      );
